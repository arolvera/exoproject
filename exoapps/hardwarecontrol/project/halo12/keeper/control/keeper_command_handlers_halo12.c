...
#include "keeper_mcu_halo12.h.in"

static void on_off(uint8_t *data)
{
    communication_union_t *msg = (communication_union_t*)data;
    if(msg->command.set_point == ON_SET_POINT && keeper.common.current_state == OFF_STATE)
    {
        if(msg->command.specifier == RESTART)
        {
            keeper.common.current_state = keeper_restart_handler();
        } else {
            keeper.common.current_state = keeper_on_command_handler();
        }
    }
    else if(msg->command.set_point == OFF_SET_POINT && keeper.common.current_state != ERROR_STATE)
    {
        keeper.common.current_state = keeper_off_command_handler();
    } else {
        cc_error_report(INVALID_STATE, 0, COMM_ID_KEEPER);
    }
}
...
static void set_current(uint8_t *data)
{
    ...
    if(msg->command.set_point < KEEPER_MAX_I_OUT_COUNTS)
    {
        ...
        cc_error_report(INVALID_SETPOINT, 0, COMM_ID_KEEPER);
    }
}
