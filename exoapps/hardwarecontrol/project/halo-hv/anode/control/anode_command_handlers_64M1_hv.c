...
static void on_off(uint8_t *data)
{
    ...
    if(msg->command.set_point == ON_SET_POINT && common.current_state == OFF_STATE && 
            anode.target_voltage < ANODE_MAX_V_OUT_COUNTS && anode.target_voltage > ANODE_MIN_V_OUT_COUNTS)
    {
        common.current_state = anode_on_command_handler();
    }
    else if(msg->command.set_point == OFF_SET_POINT && common.current_state != ERROR_STATE)
    {
        common.current_state = anode_off_command_handler();
    } else {
        cc_invalid_state();
    }
}
